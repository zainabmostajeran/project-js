...
async function fetchSneakerInfo() {
  try {
    ...
    let render = response.data.map((el, index) => {
      return generateRowSneakerInfo(el, index);
    });
...
function generateRowSneakerInfo(sneaker, index) {
  ...
  <div class="card flex flex-col gap-2 items-center justify-start" data-index="${index}">
    <div class="rounded-2xl w-40 h-40  overflow-hidden">
...
async function fetchBrands() {
  try {
    ...
    btnBrand.innerHTML=`<button class="filter border border-black rounded-full px-7 py-1 font-semibold text-base" data-filter="All">All</button>`
    + renderBrand.join("");
    handleBrandFilter();
  } catch (error) {
    errorHandler(error);
...
function generateBrand(brand, index) {
  return `<button
  class=" border border-5 border-black  rounded-full px-5 py-1 font-bold text-base " data-index="${index}"
